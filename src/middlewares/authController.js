...
const register = async (req, res, next) => {
  try {
    ...
    const user = await User.create({ organisation_id: org.id, email, password_hash, name: adminName });

    await audit.log(user.id, org.id, 'org.create', { orgId: org.id, userId: user.id });
    const token = jwt.sign({ userId: user.id, orgId: org.id }, process.env.JWT_SECRET, { expiresIn: '8h' });
    res.status(201).json({ token, user: { id: user.id, email: user.email, name: user.name, orgId: org.id } });
  } catch (err) { next(err); }
};
...
const logout = async (req, res, next) => {
  try {
    // stateless JWT: we log logout but token invalidation would need blacklist (optional)
    const user = req.user;
    await audit.log(user.id, user.orgId, 'auth.logout', {});
    res.json({ ok: true });
  } catch (err) { next(err); }
};
